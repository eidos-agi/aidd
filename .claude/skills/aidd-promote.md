...
5. If the product has a native Apple surface, run the plugin that matches it: `/aidd-mac`, `/aidd-iphone`, `/aidd-ipad`. Mechanical gates first. A green suite on one surface is inadmissible on the others. The builder does not approve its own UI.
